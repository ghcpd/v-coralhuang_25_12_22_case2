"""
Workload runner for testing API handlers with concurrent execution.
Executes handlers directly (not via HTTP) to simulate in-process load testing.
"""
import json
import time
import statistics
...
def run_workload(config, run_label='baseline', optimization_profile=None):
    """
    Execute the full workload with concurrent requests.
    
    Args:
        config: Configuration dict from input.json
        run_label: Label for this run (baseline/optimized)
        optimization_profile: Optional dict to track which optimizations are enabled
    ...
    with ThreadPoolExecutor(max_workers=concurrency) as executor:
        ...
        for i in range(warmup_requests):
            ...
            future = executor.submit(_execute_request, req)
...
def _execute_request(req_config):
    ...
    try:
        ...
        try:
            ...
            if endpoint == '/api/users':
                ...
                response, status_code = get_users(page=page, per_page=per_page, auth_header=auth_header)
            elif '/followers' in endpoint:
                user_id = int(endpoint.split('/')[3])
                page = query.get('page', 1)
                per_page = query.get('per_page', 100)
                response, status_code = get_user_followers(user_id, page=page, per_page=per_page, auth_header=auth_header)
            else:
...
def main():
    """Main entry point for workload runner."""
    ...
    # Run baseline
    baseline_results = run_workload(config, run_label='baseline')
